...
void DrawDiffBoot(){
  ...
  for(int irun=0; irun<1; irun++){
    ...
    //tree->AddFile(Form("rootfiles/BootStrap/det0_Noise/G4AnaData%04d.root",irun),0,"tree");
    tree->AddFile(Form("rootfiles/BootStrap/det0_Dev/G4AnaData%04d.root",irun),0,"tree");
  }
  ...
  TH1D *hdiff[3];
  hdiff[0] = new TH1D("hdiff0","rdiffphi",15,-10,10);
  hdiff[1] = new TH1D("hdiff1","diffr",15,-10,10);
  hdiff[2] = new TH1D("hdiff2","diffz",15,-10,10);
  ...
  for(ievt=0; ievt<nentries; ievt++){
    ...
    for(int i=0; i<repeat; i++){
      ...
      double diffphi = anaphi-bootphi;
      if(diffphi>TMath::Pi()) diffphi-= 2*TMath::Pi();
      if(diffphi<-TMath::Pi()) diffphi+= 2*TMath::Pi();
    
      hdist->Fill(dist);
      hdiff[0]->Fill(bootr*diffphi);
      hdiff[1]->Fill(anar-bootr);
      hdiff[2]->Fill(anapos[2]-bootpos[i][2]);
    }
  }
  ...
  hdiff[0]->GetYaxis()->SetRangeUser(0,30e5);
