...
// Add the properly scaled and shifted bestPoint to PE.rAmplitude
void PSAFilterGridSearch::AddToSolution(pointExp &PE, pointPsa &bestPoint, float scaleFact){
  float *realTrace;
  float *baseTrace;
  for(int segm = 0; segm < NCHAN; segm++){
    realTrace = PE.rAmp[segm];
    baseTrace = bestPoint.Amp[segm];
    for(int nn = 0; nn < BSIZE; nn++){
      (*realTrace++) += float(*baseTrace++)*scaleFact;
    }
  }
}
