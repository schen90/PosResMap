...
void AnaData(string inputfile, string outputfile, int detid, string basisfile){
  ...
  anatree->Branch("bootpos", bootpos, Form("bootpos[%d][3]/F",repeat));
  ...
  for(ievt=0; ievt<nentries; ievt++){
	...
	pointExp pB;
	pB.numNetCharges = pE.numNetCharges;
	pB.netChSeg = -1;
	pB.bestPt = -1;
	pB.chi2min = float(1.e30);
	pB.isValid = false;
	pB.isInitialized = false;
	memcpy(pB.netChargeSegnum, pE.netChargeSegnum, sizeof(pB.netChargeSegnum));
	memcpy(pB.netChargeEnergy, pE.netChargeEnergy, sizeof(pB.netChargeEnergy));

	memcpy(pB.nhits, pE.nhits, sizeof(pB.nhits));
	memcpy(pB.x, pE.x, sizeof(pB.x));
	memcpy(pB.y, pE.y, sizeof(pB.y));
	memcpy(pB.z, pE.z, sizeof(pB.z));
	
	for(int iseg=0; iseg<NCHAN; iseg++){
	  vector<float> residual;
	  for(int isig=0; isig<BSIZE; isig++){
	    residual.push_back(pE.wAmp[iseg][isig]);
	  }
	...
	for(int i=0; i<repeat; i++){
	  bestPt = fpB[i].resPt[snum];
	  fpsa->GetPtPos(seg, bestPt, bootpos[i]);
	}
