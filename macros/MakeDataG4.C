...
void MakeDataG4(string G4inputfile, string outputfile){
  ...
  while(!feof(fp)){ // loop events in G4inputfile
    ...
    if(EoF){//check next file exist

      fileidx++;
      string G4inputfile2 = (string)Form("%s.%02d",G4inputfile.c_str(),fileidx);
      if(!gSystem->AccessPathName(G4inputfile2.c_str())){
	fclose(fp);
	fp = fopen(G4inputfile2.c_str(),"r");
	if(!fp){  printf("could not open file %s\n",G4inputfile2.c_str()); return;}
	printf("\e[1;33m opened file %s \e[0m\n",G4inputfile2.c_str());
